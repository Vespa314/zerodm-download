...
def ShowEpsInfo(EPSInfo):
    ...
    4 in a row
    """
    eps_list = list(map(lambda x:x[1],EPSInfo));
    m_str = "";
...
def GetChoice():
    while True:
        inputrange = input("Input Download Range:(For Example:2-10 or 17 or all)");
...
def GetEpsInfo(id):
    ...
    return GetRE(content.decode('GBK'),regexp);
...
def GetDLURL_xunlei(pageurl):
    try:
        ...
        return [];
    regexp = b"href=['\"](http.+?sendfile.+?)['\"].*?>";
    urllist = GetRE(content,regexp);
    downloadlist = [];
    if len(urllist) == 0:
        return [];
    for url in urllist:
        downloadlist.append(url.decode());
    return downloadlist;
...
def DownLoad(EPSInfo,eps_range,animate_name):
    ...
    for i in range(len(EPSInfo)):
        if i+1 in eps_range:
            print("Getting Download URL for "+EPSInfo[i][1]+":",end="");
